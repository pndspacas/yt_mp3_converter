...
# Detecta a pasta Downloads ou Transferências
...
def parse_time(t):
    """Converte 'mm:ss' ou 'ss' em segundos inteiros"""
    if not t:
        return None
    if ":" in t:
        m, s = t.split(":")
...
if st.button("Baixar MP3"):
    if not url:
        st.warning("⚠️ Por favor, insira o link do YouTube.")
    else:
